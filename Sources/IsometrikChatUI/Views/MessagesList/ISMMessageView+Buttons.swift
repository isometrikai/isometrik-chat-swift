    ...
    func scrollToBottomButton() -> some View {
        return Button(action: {
            ...
            if let lastMessageId = self.realmManager.messages.last?.last?.id.description {
                ...
                // Append timestamp to force onChange to trigger even if messageId is the same
                parentMessageIdToScroll = "\(lastMessageId)_\(Date().timeIntervalSince1970)"
                // Then immediately set to actual messageId (onChange will handle the scroll)
                ...
                Button {
                    ...
                    if ISMChatSdkUI.getInstance().getChatProperties().navigateToAppProfileFromMessageList == true {
                        if (isGroup ?? false) == true {
                            ...
                            delegate?.navigateToAppProfile(userId: userId ?? "", storeId: storeId ?? "", userType: self.conversationDetail?.conversationDetails?.opponentDetails?.metaData?.userType ?? 0,conversationID: self.conversationID ?? "")
                        }
                    } else {
                        self.stateViewModel.navigateToUserProfile = true
                    }
