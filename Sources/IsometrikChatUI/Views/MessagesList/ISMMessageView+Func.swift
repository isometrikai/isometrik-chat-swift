...
//                localIds.removeFirst()
            ...
            var localIds = [UUID]()
            let sentAt = Date().timeIntervalSince1970 * 1000
            let id = await saveMessageToLocalDB(sentAt: sentAt, messageId: "", message: msg, mentionUsers: [], fileName: mediaName, fileUrl: "", messageType: messageKind,customType: messageKind == .video ? .Video : .Image, messageKind: .normal,mediaCaption: cameraCaption)
            localIds.append(id)
            ...
            if messageKind == .video , let videoUrl = videoUrl{
                    ...
                    if ISMChatSdk.getInstance().checkuploadOnExternalCDN() == true{
                        self.delegate?.uploadOnExternalCDN(messageKind: .photo, mediaUrl: thumbnailUrl!, completion: { imageURL, _ in
                            self.delegate?.uploadOnExternalCDN(messageKind: .video, mediaUrl: videoUrl, completion: { videoUrl, mediaData in
                                sendMediaMessage(messageKind: messageKind, customType: customType, mediaId: mediaId, mediaName: mediaName, mediaUrl: videoUrl, mediaData: mediaData, thubnailUrl: imageURL, sentAt: sentAt, objectId: localIds.first ?? UUID(), caption: cameraCaption)
                                localIds.removeFirst()
                            })
                        })
                    }else{
                        ...
                        //upload video
                            ...
                            if let dataValue = data {
                                sendMediaMessage(messageKind: messageKind, customType: customType, mediaId: dataValue.mediaId ?? "", mediaName: filename, mediaUrl: data?.mediaUrl ?? "", mediaData: size, thubnailUrl: thumbnailmedia?.mediaUrl ?? "", sentAt: sentAt, objectId: localIds.first ?? UUID(), caption: cameraCaption)
                                localIds.removeFirst()
                            }
                        }
                    }
                ...
                if ISMChatSdk.getInstance().checkuploadOnExternalCDN() == true{
                    self.delegate?.uploadOnExternalCDN(messageKind: .photo, mediaUrl: cameraImage, completion: { imageURL, imageData in
                        sendMediaMessage(messageKind: .photo, customType: ISMChatMediaType.Image.value, mediaId: mediaId, mediaName: mediaName, mediaUrl: imageURL, mediaData: imageData, thubnailUrl: imageURL, sentAt: sentAt, objectId: localIds.first ?? UUID(), caption: cameraCaption)
                        localIds.removeFirst()
                    })
                }else{
                    chatViewModel.upload(messageKind: .photo, conversationId: self.conversationID ?? "", image: cameraImage, document: nil, video: nil, audio: nil, mediaName: mediaName) {  data, filename, size in
                        if let data = data {
                            sendMediaMessage(messageKind: .photo, customType: ISMChatMediaType.Image.value, mediaId: data.mediaId ?? "", mediaName: filename, mediaUrl: data.mediaUrl ?? "", mediaData: size, thubnailUrl: data.thumbnailUrl ?? "", sentAt: sentAt, objectId: localIds.first ?? UUID(), caption: cameraCaption)
                            localIds.removeFirst()
                        }
                    }
                }
...
//                parentMessageIdToScroll = id ?? ""
    ...
    func sendMediaMessage(messageKind : ISMChatMessageType,customType : String,mediaId : String,mediaName: String,mediaUrl : String,mediaData: Int,thubnailUrl : String,sentAt : Double,objectId : UUID,caption : String){
        chatViewModel.sendMessage(messageKind: messageKind, customType: customType, conversationId: self.conversationID ?? "", message: mediaUrl, fileName: mediaName, fileSize: mediaData, mediaId: mediaId,thumbnailUrl: thubnailUrl,caption: caption) {messageId,_ in
            ...
            Task{
                // update message Id as well as media
                ...
                // scroll to lastly added message
                parentMessageIdToScroll = self.viewModelNew.messages.last?.last?.id.description ?? ""
                if messageKind != .audio && messageKind != .sticker && messageKind != .gif{
                    let attachment = ISMChatAttachmentDB(attachmentType: ISMChatAttachmentType.Video.type, extensions: ISMChatExtensionType.Video.type, mediaUrl: mediaUrl, mimeType: ISMChatExtensionType.Video.type, name: mediaName, thumbnailUrl: thubnailUrl)
                    // save media in conversation
                    await viewModelNew.saveMedia(arr: [attachment], conversationId: self.conversationID ?? "", customType: customType, sentAt: sentAt, messageId: messageId, userName: userData?.userName ?? "")
