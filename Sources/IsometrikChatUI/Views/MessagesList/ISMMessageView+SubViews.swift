    ...
    func defaultMessageView(message : ISMChatMessagesDB,scrollReader : ScrollViewProxy,viewWidth : CGFloat) -> some View{
        HStack{
            ...
            ISMMessageSubView(messageType: ISMChatHelper.getMessageType(message: message),
                              ...
                              messageDeliveredType: ISMChatHelper.checkMessageDeliveryType(message: message,
                                                                                           isGroup: self.isGroup ?? false,memberCount: groupMemberCount,
                                                                                           isOneToOneGroup: ISMChatSdkUI.getInstance().getChatProperties().isOneToOneGroup),
                              ...
                              memberCount: self.groupMemberCount,
...
//            .environmentObject(self.realmManager)
    ...
    func getIsReceived(message: ISMChatMessagesDB) -> Bool {
        ...
        if message.customType == ISMChatMediaType.AudioCall.value || message.customType == ISMChatMediaType.VideoCall.value{
            return message.initiatorId != myUserId
        }else{
            ...
            return senderInfo.userId ?? message.initiatorId != myUserId
        }
    }
