    ...
    func grpHeader(action: ISMChatActionType, userName: String, senderId: String,member : String? = nil,memberId : String? = nil,isGroup : Bool? = true) -> some View {
        ZStack {
            ...
            if action == .userBlock{
                ...
                let text = senderId == userId ? "You changed this group title".localized : "\(userName) changed this group title"
                customText(text: text)
            }else if action == .conversationImageUpdated{
                let text = senderId == userId ? "You changed this group image".localized : "\(userName) changed this group image"
                customText(text: text)
