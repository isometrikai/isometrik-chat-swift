...
extension ISMConversationView{
    ...
    func localNotificationForActions(messageInfo: ISMChat_MessageDelivered){
    ...
    func messageDeleteForAll(messageInfo: ISMChat_MessageDelivered) {
    ...
    func blockUnblockUserEvent(messageInfo : ISMChat_UserBlockAndUnblock){
        if userSession.getUserId() != messageInfo.initiatorId{
            let msg = ISMChat_LastMessage(sentAt: messageInfo.sentAt,conversationId: messageInfo.conversationId, body: nil, messageId: messageInfo.messageId, deliveredToUser: messageInfo.opponentId, timeStamp: messageInfo.sentAt, action: messageInfo.action, initiatorId: messageInfo.initiatorId, initiatorName: messageInfo.initiatorName,initiatorIdentifier: messageInfo.initiatorIdentifier)
            
            self.realmManager.updateLastmsg(conId: messageInfo.conversationId ?? "", msg: msg)
            
            if messageInfo.action == ISMChat_ActionType.conversationCreated.value{
    ...
    func addReaction(messageInfo: ISMChat_Reactions){
        //update last message in conversationList
        let msg = ISMChat_LastMessage(sentAt: messageInfo.sentAt,conversationId: messageInfo.conversationId, body: nil, messageId: messageInfo.messageId, timeStamp: messageInfo.sentAt, action: messageInfo.action,userName: messageInfo.userName ?? "", reactionType: messageInfo.reactionType)
    ...
    func removeReaction(messageInfo: ISMChat_Reactions){
        //update last message in conversationList
        let msg = ISMChat_LastMessage(sentAt: messageInfo.sentAt,conversationId: messageInfo.conversationId, body: nil, messageId: messageInfo.messageId, timeStamp: messageInfo.sentAt, action: messageInfo.action,userName: messageInfo.userName ?? "", reactionType: messageInfo.reactionType)
    ...
    func msgReceived(messageInfo: ISMChat_MessageDelivered) {
    ...
    func msgDelivered(messageInfo: ISMChat_MessageDelivered) {
        if userSession.getUserId() != messageInfo.senderId {
    ...
    func messageRead(messageInfo : ISMChat_MultipleMessageRead){
    ...
    func typingStatus(obj: ISMChat_TypingEvent) {
