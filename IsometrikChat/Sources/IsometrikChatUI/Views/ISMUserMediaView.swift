...
struct ISMUserMediaView: View {
        ...
        LazyVGrid(
            ...
            Section(header: Text(key.toString(dateFormat: "dd MMM yyyy"))
                ...
                ForEach(contacts) { value in
                    if value.customType == ISMChat_MediaType.Video.value {
                        showVideoView(value)
                    } else if value.customType == ISMChat_MediaType.Image.value {
                        showImageView(value)
                    } else if value.customType == ISMChat_MediaType.File.value {
                        showFileView(value)
                    } else if value.customType == ISMChat_MediaType.gif.value{
                        showGifView(value)
                    } else {
    ...
    func showImageView(_ value: MediaDB) -> some View {
        NavigationLink(
            ...
            ISMChat_ImageCahcingManger.networkImage(url: value.mediaUrl ,isprofileImage: false)
                .resizable()
                .scaledToFill()
                .frame(width: ((UIScreen.main.bounds.width / 3) - 1), height: ((UIScreen.main.bounds.width / 3) - 1))
                .clipped()
