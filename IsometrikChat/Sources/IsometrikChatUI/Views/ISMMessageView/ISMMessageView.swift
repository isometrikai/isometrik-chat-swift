...
//    public var delegate : MessageVCDelegate?
    ...
    @State var parentMessage : ISMChat_Message = ISMChat_Message()
...
//    @State var selectedGIF: GPHMedia?
    ...
    public var body: some View {
        ...
        .onReceive(NotificationCenter.default.publisher(for: ISMChat_MQTTNotificationType.mqttMessageNewReceived.name)){ notification in
            ...
            ISMChat_Helper.print("MESSAGE RECEIVED----------------->\(messageInfo)")
            //save in local db
        ...
        .onReceive(NotificationCenter.default.publisher(for: ISMChat_MQTTNotificationType.mqttMessageRead.name)){ notification in
            guard let messageInfo = notification.userInfo?["data"] as? ISMChat_MessageDelivered else {
                return
            }
            ISMChat_Helper.print("MESSAGE READ ----------------->\(messageInfo)")
        ...
        .onReceive(NotificationCenter.default.publisher(for: ISMChat_MQTTNotificationType.mqttUserBlock.name)){
            notification in
        ...
        .onReceive(NotificationCenter.default.publisher(for: ISMChat_MQTTNotificationType.mqttUserUnblock.name)){
            notification in
            guard let messageInfo = notification.userInfo?["data"] as? ISMChat_UserBlockAndUnblock else {
                return
            }
...
//        }
        ...
        .onReceive(NotificationCenter.default.publisher(for: ISMChat_MQTTNotificationType.mqttMultipleMessageRead.name)){ notification in
            ...
            ISMChat_Helper.print("MULTIPLE MESSAGE READ ----------------->\(messageInfo)")
...
//        .background(NavigationLink("", destination: ISMChat_BroadCastInfo(broadcastTitle: (self.groupConversationTitle ?? ""),groupCastId: self.groupCastId ?? "").environmentObject(self.realmManager),isActive: $navigateToGroupCastInfo))
        ...
        if self.realmManager.messages.count > 0 {
            if (self.realmManager.messages.last?.count ?? 0) > 0 {
                if let msgObj = self.realmManager.messages.last?.last {
                    ...
                    if action != ISMChat_ActionType.reactionAdd.value && action != ISMChat_ActionType.reactionRemove.value{
...
//        }
    ...
    func scrollTo(messageId: String, anchor: UnitPoint? = nil, shouldAnimate: Bool, scrollReader: ScrollViewProxy) {
        DispatchQueue.main.async {
            ISMChat_Helper.print("Scrolling to messageId: \(messageId)")
