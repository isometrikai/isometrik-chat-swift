...
extension ISMMessageView{
    ...
    func AudioMessageButton(height : CGFloat) -> some View{
        HStack{
            Button(action: {
                ISMChat_Helper.print("recording done")
                if isClicked == true{
            ...
            .simultaneousGesture(
                LongPressGesture(minimumDuration: 0.1)
                    .onEnded { value in
                        ISMChat_Helper.print("Tap currently holded")
                        if isMessagingEnabled() == true && viewModel.isBusy == false{
                            if audioPermissionCheck == true{
                                ...
                                ISMChat_Helper.print("Access Denied for audio permission")
                            }
                        }
                    }
                    .sequenced(before:
                    ...
                    Button {
                        ...
                        if let groupConversationTitle = groupConversationTitle, !groupConversationTitle.isEmpty && groupConversationTitle != "Default"{
                            Text(groupConversationTitle)
                                .foregroundColor(themeColor.messageList_HeaderTitle)
                                .font(themeFonts.messageList_HeaderTitle)
                        }else{
                            Text("Messages")
                                .foregroundColor(themeColor.messageList_HeaderTitle)
                                .font(themeFonts.messageList_HeaderTitle)
                            ...
                            if isGroup == true{
                                if otherUserTyping == true{
                                    Text("\(typingUserName ?? "") is typing...")
                                        .foregroundColor(themeColor.messageList_HeaderDescription)
                                        .font(themeFonts.messageList_HeaderDescription)
